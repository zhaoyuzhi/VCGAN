    ...
    for i, classname in enumerate(classlist):
        for j, imgname in enumerate(wholepathlist):
            if imgname.split('/')[-2] == classname:
                imgname = os.path.join(opt.basepath, 'videvo', imgname)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--imagelist_txt', type = str, default = './videvo_test_imagelist_without_first_frame.txt', help = 'the path contains all the relative image names')
