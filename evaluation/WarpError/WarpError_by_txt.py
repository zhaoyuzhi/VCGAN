    ...
    for i, classname in enumerate(classlist):
        for j, imgname in enumerate(wholepathlist):
            if imgname.split('/')[-2] == classname:
                imgname = os.path.join(opt.base_path, imgname)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--imagelist_txt', type = str, default = './DAVIS_test_imagelist.txt', help = 'the path contains all the relative image names')
    ...
    parser.add_argument('--report', type = bool, default = True, help = 'whether the results should be saved')
    ...
    if opt.report:
        content = '%s \t wrap error: %8f' % (os.path.split(opt.base_path)[-2], avg_loss.item())
