...
def trainer_noGAN(opt):
    ...
    for epoch in range(opt.epochs):
        for i, (true_L, true_RGB) in enumerate(dataloader):
            ...
            # Print log
            print("\r[Epoch %d/%d] [Batch %d/%d] [Pixel-level Loss: %.4f] [Perceptual Loss: %.4f] Time_left: %s" %
                ((epoch + 1), opt.epochs, i, len(dataloader), loss_L1.item(), loss_percep.item(), time_left))
...
def trainer_LSGAN(opt):
    ...
    for epoch in range(opt.epochs):
        for i, (true_L, true_RGB) in enumerate(dataloader):
            ...
            # Print log
            print("\r[Epoch %d/%d] [Batch %d/%d] [Pixel-level Loss: %.4f] [Perceptual Loss: %.4f] [D Loss: %.4f] [G Loss: %.4f] Time_left: %s" %
